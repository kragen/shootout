...
<p>
  ...
  href="../../method.shtml#sameway"><i>same way</i></a>. (For this
  ...
  but that would make it a different test.)
<p>
...
<p>
  ...
  the <a href="detail.shtml">detail page</a> to see results for
  different values of N.  Those languages that do not implement
  tail recursion elimination (tail-call elimination) will not
  peform as well as those that do.
<p>
  This test also appears in the Kernighan and Wyk study <a
...
<h4>Observations</h4>
<p>
  The <a href="ackermann.bash">bash program</a> is disqualified
  because it exceeds the time limit (300 CPU seconds).
<p>
  The <a href="ackermann.guile">guile program</a> fails from stack
  overflow unless I add <code>(debug-set! stack 0)</code>.
<p>
  The <a href="ackermann.tcl">Tcl program</a> fails from stack
  overflow.
<p>
  The <a href="ackermann.bigforth">bigforth program</a> dumps core.
<p>
  I was a little surprised by Perl's bad performance and memory usage
  ...
  recursion is so slow.</i>

<h4>About Ackermann's Function</h4>
<p>
...
<p>
  From: <a href="http://pweb.netcom.com/~hjsmith/Ackerman.html">Harry
  J. Smith's web page about the Ackermann Function:</a>

<p>
  <i>The Ackermann function is the simplest example of a well-defined
...
<ul>
<li>
  I find Forth a little hard to read, so I have an annotated version
  of the Forth ackermann's function program <a
  href="alt/ann.ackermann.forth">here</a>.
<li>
  The <a href="ackermann.perl">Perl program</a> is quite bad at
  this type of heavy recursion, so I wrote a second version <a
  href="alt/ackermann.perl2.perl">perl2</a> which saves intermediate
  ...
  keep kids and pets out of the blast area.  
<li>
  Marcus Comstedt contributed a <a href="alt/ackermann.pike2.pike">
  Pike program </a> that uses memoization, and he also contributed
  another <a href="alt/ackermann.pike3.pike">Pike program</a> that
  avoids the recursive call altogether :-)
<li>
  Johan Boulé contributed a <a href="alt/ackermann.se2.se">SmallEiffel</a>
  that converts some of the recursive calls to an iterative implementation.
<li>
  Michael A. Cleverly submitted a <a href="alt/ackermann.tcl2.tcl">Tcl
  ...
  remembered.  Some functional languages work like this automatically.
</ul>
