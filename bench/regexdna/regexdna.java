...
public class regexdna
{
            ...
            pool[i] = new Thread()
            {
                public void run()
                {
                    ...
                    for (int pt = 0; pt < patterns.length; pt++)
                    {
                        ...
                        {
                            Matcher mt = expression.matcher(source_as_segments.get(seq));
                            while (mt.find())
    ...
    {
        final String[] pat_search = {"W", "Y", "K", "M", 
            "S", "R", "B", "D", 
            "V", "H", "N"};
        final String[] pat_replace = {"(a|t)", "(c|t)", "(g|t)", "(a|c)", 
            "(c|g)", "(a|g)", "(c|g|t)", "(a|g|t)", 
            "(a|c|g)", "(a|c|t)", "(a|c|g|t)"};
                ...
                public void run()
                {
                    for (int pt = 0; pt < pat_search.length; pt++)
                    {
                        ...
                        while ((line = tasks.getAndIncrement(pt)) < total_line)
                        {
                            ...
                            Matcher m = pattern.matcher(src_buf);
