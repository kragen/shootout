...
/*                                                                    */
/* Adapted from Pike version by Adam Montague                         */
/*                                                                    */
...
object nsieve
{
  ...
  {
    val sieve: Array[byte] = new Array[byte](m);

    var i:int = 0; while (i < m) { sieve(i) = 1; i = i + 1; }
    ...
    while (i < m)
    {
      if (sieve(i) != 0)
      {
        var j: int = i + i; while (j < m) { sieve(j) = 0; j = j + i; }
        count = count + 1;
      }
