...
<p>This test is basically the same as the associative array test from <a
  ...
  find this test pretty meaningless.</p>
<p>To make the point about sprintf(), I rewrote the Ocaml program so
...
<p>The <a href="hash.gcc">C program</a> uses a simple Hash Table
  implemented in this header file: <a href="../Include/simple_hash.h">
  simple_hash.h</a>.</p>
<p>Jochen Schmidt contributed a <a href="hash.cmucl">Common Lisp
  program</a> that uses a custom formatting function instead of
  <i>princ-to-string</i>, for a five-fold overall speedup.</p>
...
<ul>
<!-- link died
<li>Based on previous work by Jacques Bouchard we can vastly improve the
  ...
  key does not fail.</li>
 -->
<li>Mark Baker contributed another <a href="alt/hash.python2.python">
  Python</a> program that map, filter, and len to improve performance.
  This solution is faster, but not enough to improve Python's rank.  It
  also uses a little more memory, but it's nice and short and a good example
  of using these higher order functions.  Since it doesn't really implement
  the same operations as the other programs (same way), I've put it in the
  alternates section for now.</li>
</ul>
