...
<p>This test compares the speed of exceptions as implemented in
  different languages.  Each test throws N exceptions.  Visit the <a
  href="detail.php">detail page</a> to see results for different
  values of N.</p>
<p>Each test program throws a bunch of exceptions, half of which are caught
...
<p>This is a fairly meaningless test in terms of the measured results
...
<h4>Observations</h4>
<p>In Ruby, if an exception occurs, the code that caused the exception
  can easily be restarted with <b>retry</b>, which is a neat feature.</p>
<p>Rep exceptions are fast ... but we are not really instantiating
  exception objects and throwing them, we are only throwing a tag and
  value, which is pretty much like the Lisp equivalent of a GOTO.</p>

<p>I think that using exceptions to implement normal program flow is bad
  ...
  need to find my old copy to make sure of this).</p>

<p>When compiled with -O3, the G++ (2.95.3) program dumps core.  It
  runs fine compiled with -O2.</p>
...
<ul>
  <li>Mark Baker contributed a slightly faster <a href=
  "alt/except.python2.python">Python</a> program, however, I've
  ...
  restriction, I'll use this version instead of the current one.</li>
</ul>
