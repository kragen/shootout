...
public class fannkuch implements Runnable
{
    ...
    private static int[] flip_max_arr;
    private static AtomicInteger remain_task = new AtomicInteger(0);
    
    public static void main(String[] args)
    {
        n = (args.length > 0) ? Integer.parseInt(args[0]) : 7;
    ...
    private static int fank_game()
    {
        // hold flip_count result for each thread
        flip_max_arr = new int[n];
        Thread[] th = new Thread[Runtime.getRuntime().availableProcessors()];
        for (int i = 0; i < th.length; i++)
        {
            th[i] = new Thread(new fannkuch());
            th[i].start();
        }
