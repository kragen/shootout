...
# $Id: u32.programs.Makefile,v 1.39 2010-08-26 02:56:56 igouy-guest Exp $
...
%.gnat_run: %.gnat
	-$(GNATCHOP) -r -w $<
	-$(GNATC) $(GNATOPTS) $(STD_COPTS) -f $(TEST).adb -o $@ $(GNATLDOPTS)
...
%.fsharp_run: %.fs
	-$(MONORUN) $(FSHARPC) --platform:x86 -O $(FSHARPOPTS) -o $@.exe $<
...
%.clojure_run: %.clojure $(CLOJURE)
	-mv $< $(TEST).clj
#	-$(JDKRUN) -cp .:$(CLOJURE):$(CLOJURECONTRIB) -Dclojure.compile.path=. clojure.lang.Compile $(TEST)
